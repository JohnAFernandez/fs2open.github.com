...
void send_beam_fired_packet(const beam_fire_info *fire_info, const beam_info *override)
{
	...
	// setup outgoing data
	Assert(fire_info->shooter != nullptr);
	Assert(fire_info->turret != nullptr);
	Assert(override != nullptr);

	if ( (fire_info->shooter == nullptr) || (fire_info->turret == nullptr) || (override == nullptr) ) {
		return;
	}
	...
	if (fire_info->bfi_flags & BFIF_IS_FIGHTER_BEAM) {
		...
		Assertion(fire_info->target != nullptr, "BEAM fired without a target!");
	...
	ADD_USHORT(fire_info->shooter->net_signature);
	ADD_USHORT(target_sig);
	ADD_SHORT(u_beam_info);
	ADD_FLOAT(fire_info->accuracy);
...
void process_beam_fired_packet(ubyte *data, header *hinfo)
{
	...
	if ( (fire_info.shooter == nullptr) || (fire_info.shooter->type != OBJ_SHIP) || (fire_info.shooter->instance < 0) || (fire_info.shooter->instance >= MAX_SHIPS) ) {
	...
	if (target_subsys_index >= 0) {
		fire_info.target_subsys = ship_get_indexed_subsys(shipp, target_subsys_index);
	}
