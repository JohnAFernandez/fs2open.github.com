...
#define GET_STRING(s) do { uint8_t len; memcpy(&len, data+offset, sizeof(len)); offset += sizeof(len); memcpy(s, data+offset, len); offset += len; s[len] = '\0'; } while (false)
#define GET_STRING_16(s) do { uint16_t len; GET_USHORT(len); offset += sizeof(len); memcpy(s, data+offset, len); offset += len; s[len] = '\0'; } while (false)
