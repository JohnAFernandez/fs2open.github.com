...
ubyte multi_pack_unpack_desired_vel_and_desired_rotvel( int write, bool full_physics, ubyte *data, physics_info *pi, vec3d* local_desired_vel)
{
	...
	if (write) {
		...
		if (full_physics) {
			...
			pi->desired_rotvel.xyz.x = pi->max_rotvel.xyz.x * i2fl(a) / 31.0f;
			pi->desired_rotvel.xyz.y = pi->max_rotvel.xyz.y * i2fl(b) / 31.0f;
			pi->desired_rotvel.xyz.z = pi->max_rotvel.xyz.z * i2fl(c) / 7.0f;
		}
